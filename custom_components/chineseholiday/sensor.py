...
class ChineseHolidaySensor(Entity):
    ...
    def _update(self):
        ...
        # 合并 holiday.py 的 attributes 到 localizedAttributes
        for k, v in self._holiday.attributes.items():
            if k not in self.localizedAttributes:
                self.localizedAttributes[k] = v
